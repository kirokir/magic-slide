<script lang="ts">
	...
	function handleGenerate() {
		if (!generatorText.trim()) {
			showToast('Please enter text to generate slides.', 'error');
			return;
		}
		try {
			const newSlides = parseTextToSlides(generatorText, get(globalSettingsStore).brandingKit.brandColor);
			if (newSlides.length === 0) {
				showToast('No slides could be generated. Check your text format.', 'warning');
				return;
			}
			...
			showToast(`Generated ${newSlides.length} new slide(s).`, 'success');
		} catch (error) {
			console.error('Error generating slides:', error);
			showToast('Error generating slides. Please check console.', 'error');
		}
	}
